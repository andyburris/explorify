import { CombineInto, CombineType, Operations, GroupOperation, GroupSortType, ItemSortType, SortOperation, FilterOperation, SkipFilterType } from "./model/Operations";
...
function combineItems(items: HistoryEntry[], groupOperation: GroupOperation): Combination[] {
    const combinationMap: Map<string, HistoryEntry[]> = items.reduce((groups, entry) => {
        const key = (groupOperation.combineBy == CombineType.None) ? entry.id : (groupOperation.combineBy == CombineType.SameArtist) ? entry.artistName : `${entry.trackName} - ${entry.artistName}`
    ...
    return Array.from(combinationMap).map(([key, entries]) => {
        ...
        if(groupOperation.combineBy == CombineType.SameArtist) return new ArtistCombination(key, entries)
